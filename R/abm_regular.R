abm_regular <- function(days, delta_t, y, pars, temp_C_fun = temp_C_fun, pH_fun = pH_fun, SO4_fun = SO4_fun) {
  ...
  for (i in 1:n_int) {
    ...
    if (wash[i]) {
      ...
      if (pars$rest_d > 0) {
        ...
        outr <- deSolve::lsoda(y = y, times = times, rates, parms = parsr, temp_C_fun = temp_C_fun, pH_fun = pH_fun, SO4_fun = SO4_fun)
        # Extract new state variable vector from last row
