rates <- function(t, y, parms, temp_C_fun = temp_C_fun, pH_fun = pH_fun, 
    ...
    derivatives <- c(
       ...
       VSd_A = - VSd_A * ((exp(lnA[['VSd_A']] - E[['VSd_A']] / (R * temp_K))) * 24 / 1000 * VS_CH4) + slurry_prod_rate * conc_fresh[['VSd_A']],
       VSnd_A = slurry_prod_rate * conc_fresh[['VSnd_A']],
       CH4_A_emis_cum = VSd_A * (exp(lnA[['VSd_A']] - E[['VSd_A']] / (R * temp_K))) * 24 / 1000,
