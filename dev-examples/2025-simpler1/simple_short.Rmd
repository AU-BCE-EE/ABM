...
# Overview
This is a simple demo showing a proposal for simpler user of `abm()`.
...
It complicates the programming problem and the predicted dynamics are approximate at best.
Now the programming work is done but the speed still suffers when these processes are included.
I don't much like ammonia emission or respiration either.
And the degree or precision with which we predict inhibition is incongrous with available measurements and consistency across microbial communiities.
It could always be added through parameter values instead.

Anyway, here is an example that follows this perspective.
It is common for slurry addition rate and temperature to vary, so I'll use the variable approach.
This will be for a slurry storage tank with a 30 m diameter and 4 m depth.
So maximum volume is 2800 t (`15^2 * pi * 4`).

```{r}
var_dat <- data.frame(time = c(0, 200, 250, 300, 340, 365), 
		      slurry_mass = c(1000, 28000, 100, 12000, 1000, 2000) * 1000,
		      temp_C = c(10, 12, 15, 12, 11, 11))
...
The `all` keyword still exists in the code but there is no reason to use it.
So I plan to cut it from documentation.
