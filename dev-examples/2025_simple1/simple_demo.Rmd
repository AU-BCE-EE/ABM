...
# 11. CO2 emission
To include CO2 production from fermentation and methanogenesis and subsequent emission, it needs to be added in a few places.


Fermentation product:

```{r}
sub_pars11 <- list(subs = c('VSd', 'cellulose', 'protein', 'lipids', 'urea'),
                   forms = c(VSd = 'CH4CO2', cellulose = 'C6H10O5', protein = 'C40H61O12N10',
                             lipids = 'C57H104O6', urea = 'CO(NH2)2'),
                   T_opt_hyd = c(all = 60),
                   T_min_hyd = c(all = 0),
                   T_max_hyd = c(all = 90),
                   hydrol_opt = c(lipids = 0.1, protein = 0.01, cellulose = 0.05, urea = 1, default = 0.1),
                   sub_fresh = c(lipids = 0.1, protein = 2, cellulose = 35, urea = 10, VSd = 20),
                   sub_init = c(lipids = 0.1, protein = 2, cellulose = 35, urea = 10, VSd = 20))
...
And for emission, we need speciation (just dissolved CO2 and HCO3- here, remember 2 species limit) and a mass transfer coefficient:
...
chem_pars11 <- list(COD_conv = c(CH4 = 1/0.2507, xa = 1/0.7069561,
                                 ...
                                 CO2_aer = 1/0.436, CO2_sr = 1/1.2, 
                                 C_xa = 1/0.3753125),
                     specs = c('HCO3m', 'NH3', 'HSm', 'CH3COOm'),
                     mspec = c(HCO3m = 'CO2', NH3 = 'NH4p', HSm = 'H2S', CH3COOm = 'CH3COOH'),
                     lka = c(NH3 = '- 0.09046 - 2729.31/temp_K', 
                             HSm = '- 3448.7/temp_K + 47.479 - 7.5227 * log(temp_K)',
                             CH3COOm = '- 4.8288 + 21.42/temp_K',
                             HCO3m = '-2.778 + -353.5305 -0.06092*temp_K + 21834.37/temp_K + 126.8339*log10(temp_K) -1684915/temp_K^2'),
                     stoich = 'calc',
                     kl = c(CO2 = 1, NH3 = 0.01, H2S = 0.01, CH3COOH = 0.01) * 86400)
...
devtools::load_all()
molMass('CO(NH2)2')
molMass('CO(NH2)2', elements = 'N')
```

Here is total dissolved inorganic C:

```{r}
plot(CO2_conc ~ time, data = out11, type = 'l')
plot(CO2 ~ time, data = out11, type = 'l')
```

Units are almost nonsense.
Mass-based at the moment I guess.

And CO2 emission:

```{r}
plot(CO2_emis_cum ~ time, data = out11, type = 'l')
plot(CH4_emis_cum ~ time, data = out11, type = 'l')
plot(CH3COOH ~ time, data = out11, type = 'l', ylab = 'CO2:CH4')
plot(cellulose ~ time, data = out11, type = 'l', ylab = 'CO2:CH4')
```

CO2:CH4 ratio (cumulative):

```{r}
plot(CO2_emis_cum / CH4_emis_cum ~ time, data = out11, type = 'l', ylab = 'CO2:CH4')
plot(CO2_emis_cum / CH4_emis_cum ~ time, data = out11, type = 'l', ylab = 'CO2:CH4', ylim = c(0, 10))
```
