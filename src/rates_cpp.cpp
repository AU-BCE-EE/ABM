...
List rates_cpp(double t, NumericVector y, List parms){
  ...
  derivatives[n_mic+15] = - VSd_A * ((exp(lnA- E_CH4 / (R * temp_K))) * 24 / 1000 * 6.67) + slurry_prod_rate * conc_fresh_VSd_A;
  derivatives[n_mic+16] = - 0.01 * VSnd_A * ((exp(lnA - E_CH4 / (R * temp_K))) * 24 / 1000 * 6.67) + slurry_prod_rate * conc_fresh_VSnd_A;
  ...
  derivatives[n_mic+20] = VSd_A * (exp(lnA - E_CH4 / (R * temp_K))) * 24 / 1000;
