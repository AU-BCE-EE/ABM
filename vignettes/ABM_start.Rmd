...
We can easily make a comparison to a scenario with pH reduced to 6, with everything else the same.
...
Simply print parameter objects to check values.
...
crude protein (CP), crude fats (Cfat), residual degradable fiber (RFd), starch and sugars (starch), and even more components. 
...
An example with a simple change in VFA_concentration using the add_pars argument.
The default setting of the ABM model is also using CP, Cfat, and RFd rather than a VSd input. 
This default parameter set is called `man_pars2.0`
Since we are not using VSd input anymore when don't use grp_pars1.0 anymore, but instead the default (grp_pars2.0). 
...
```
If we use the add_pars to change a variable that is not within a sublist like VFA in "conc_fresh", we can simply type the variable directly without punctuation.  

```{r}

out2c <- abm(365, add_pars = list(slurry_prod_rate = 8000))
