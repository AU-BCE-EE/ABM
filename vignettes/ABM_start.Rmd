...
We start using parameter sets (v.1.0) that where found in Dalby et al. 2023, where substrate input is degradable VS.

```{r}
out0 <- abm(days = 365, man_pars = man_pars1.0, grp_pars = grp_pars1.0) # run the simulation for 365 days
```
¨
...
C_ratio
```
around 31% of carbon emission is methane, suggesting that a lot of carbon dioxide comes from non-methanogenic processes (urea hydrolysis and surface respiration).  
...
```
We are getting a warning now, because default settings tells the model that it should use wash water between the batches of pigs. However this is not relevant anymore since we are modelling the outside storage emission. Further when a variable slurry mass is used as input, the wash_water needs to be passed through the slurry_mass data.frame as well. Thus here it is ignored.
...
Alternatively, use `args()` just to see the arguments and default values.Here it is also seen what parameter sets are used (v1.0 or v2.0). The default parameter sets v2.0. The parameter sets can be checked out directly:
...
An example with CP, Cfat, RFd specified rather than VSd is given below and with a simple change in VFA_concentration using the add_pars argument.The default setting of the ABM model is also using CP, Cfat, and RFd rather than a VSd input. This default parameter set is called `man_pars2.0`, but below we specify it directly instead.Since we are not using VSd input anymore when don't use grp_pars1.0 anymore, but instead the default (grp_pars2.0). 
