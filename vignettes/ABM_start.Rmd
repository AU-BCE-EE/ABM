...
We will get our temperature data another file. 
...
mng_pars2c <- list(slurry_prod_rate = 5700, slurry_mass = 39000,
                   ...
                   wash_int = NA, rest_d = 5, cover = NA, resid_enrich = 0.9,
...
```{r}
out2d <- abm(365, mng_pars = mng_pars2c, add_pars = list(slurry_prod_rate = 8000))
all.equal(out2c, out2d)
```

Or to change a value in one of the default parameter sets.

```{r}
out2e <- abm(365, add_pars = list(pH = 6))
```
...
lines(CH4_emis_rate ~ time, data = out2d, col = "red")

plot(VFA_conc ~ time, data = out2d, type = 'l', col = "red", xlab = 'Time (d)', ylab = 'VFA conc. (g/kg)')
lines(VFA_conc ~ time, data = out2a)

plot(H2S_inhib_m0 ~ time, data = out2a, type = 'l', xlab = 'Time (d)', ylab = 'inhibition', ylim = c(0,1))
lines(H2S_inhib_m0 ~ time, data = out2d,  col = "red")
lines(cum_inhib_m0 ~ time, data = out2d, col = "red", lty = 2)
...
plot(CH4_emis_rate ~ time, data = out2a, type = 'l', xlab = 'Time (d)', 
     ylab = 'CH4 emis rate (g/day)')
lines(CH4_emis_rate ~ time, data = out2e, col = "red")
