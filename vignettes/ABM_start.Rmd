...
Alternatively, the complete `mgn_pars` argument can be specified.
...
An example with CP, Cfat, RFd specified rather than VSd is given below and with a simple change in VFA_concentration using the add_pars argument.
The default setting of the ABM model is also using CP, Cfat, and RFd rather than a VSd input. 
This default parameter set is called `man_pars2.0`, but below we specify it directly instead.
