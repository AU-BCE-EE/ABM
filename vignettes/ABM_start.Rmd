...
VSd_fresh
temp_dat <- outside_slurry_temp_NIR
temp_dat
```
...
plot(temp_dat$time, temp_dat$temp_C, ylab = 'Temperature (deg C)', 
     xlab = 'Time (days from 1. Jan)')
```
We need to adjust dimensions of outside slurry storage, height = 4 m, surface area = 1000
...
```{r}
out2a <- abm(365)
out2b <- abm(365, add_pars = list(slurry_prod_rate = 8000))
...
```{r}
out2c <- abm(365, mng_pars = mng_pars2c, add_pars = list(slurry_prod_rate = 8000))
