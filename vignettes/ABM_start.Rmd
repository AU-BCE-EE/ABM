...
Because we would ideally like a stabilization period, we can run the model for three years and take a look only at the last year ("startup" argument). Furthermore, we can also focus on total emission in the last year (value = "summ").

```{r}
slurry_mass_dat1a <- data.frame(time = c(slurry_mass_dat$time, slurry_mass_dat$time+365, slurry_mass_dat$time+730),
                                slurry_mass = c(rep(slurry_mass_dat$slurry_mass, 3)))

out1a <- abm(3*365, add_pars = list(storage_depth = 4, area = 1000, floor_area = 0, slurry_mass = slurry_mass_dat1a, temp_C = temp_dat, conc_fresh.VSd = VSd_fresh, conc_fresh.urea = 0), startup = 730, value = 'summ')
out1a
```
...
```{r}
out1b <- abm(3*365, add_pars = list(storage_depth = 4, area = 1000, floor_area = 0, slurry_mass = slurry_mass_dat1a, temp_C = temp_dat, conc_fresh.VSd = VSd_fresh, conc_fresh.urea = 0, pH = 5.5), startup = 730, value = 'summ')
out1b
```
