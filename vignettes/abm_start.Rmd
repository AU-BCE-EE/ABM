...
matplot(out1$time, out1[, nn <- c('m1', 'm2', 'm3', 'p1', 'p2', 'sr1')], 
        type = 'l', lty = 1, xlab = 'Time (d)', ylab = 'Microbial biomass (g)')
matlines(out4a$time, out4a[, nn <- c('m1', 'm2', 'm3', 'p1', 'p2', 'sr1')], lty = 2)
legend('topleft', nn, col = 1:6, lty = 1)
```
...
For example, to limit the groups to only `m1` and `p1`, use the following call.
Sulfate reducers are currently required!

```{r}
out4b <- abm(365, 1, add_pars = list(grps = c('m1', 'p1', 'sr1')))
head(out4b, 2)
plot(p1_conc ~ time, data = out4a, type = 'l', ylab = 'Biomass concentration (g/kg)')
lines(p1_conc ~ time, data = out1, type = 'l', col = 'red')
```
...
matplot(out5b$time, out5b[, nn <- c('m1_conc', 'm2_conc', 'm3_conc', 'p1_conc', 'p2_conc')], 
        type = 'l', lty = 1)
legend('topleft', nn, col = 1:5, lty = 1)
