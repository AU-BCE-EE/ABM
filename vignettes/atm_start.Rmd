...
out5 <- atm(365, 1, grp_pars = grp_pars)
```
...
In the ATM99 model, there are a few options for how acidification can be specified.
...
```{r}
out8 <- atm(730, 1, add_pars = list(pH = pH_dat), approx_method_pH = 'linear')
```
...
```{r}
out9 <- atm(200, 1, add_pars = list(conc_fresh.SO4 = 1.3, pH = 'calc'))
```
...
Although the `atm()` function is quite flexible and can accommodate changes in inputs over time, there is plenty it cannot do.
...
```{r}
out10 <- atm(365, 1, add_pars = list(conc_fresh.Sp = 100, conc_fresh.COD = 200, conc_fresh.VFA = 10))
```

```{r}
out11 <- atm(365, 1, add_pars = list(conc_fresh.Sp = 10, conc_fresh.COD = 20, conc_fresh.VFA = 0),
             starting = out10)
```
...
# 10. More information
Users can track development of the ATM99 package on GitHub: [https://github.com/sashahafner/ATM99](https://github.com/sashahafner/ATM99).
